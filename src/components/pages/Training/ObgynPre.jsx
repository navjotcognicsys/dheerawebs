...
import OBGYNQuestions from '../../utils/OBGYNQuestions';
import { OBGYNFilter } from '../../utils/FilterCard';
...
const ObgynPre = () => {
  ...
  const [csvfilename, setCSVfileName] = useState("obgyn_registration_pre");
  ...
  function handleFilter(options){
    ...
    let url = `https://dheeratest8.azurewebsites.net/api/v1/dheeraObgynPre/filter/${options.age}/${options.are_you_Master_Trainer_or_Trainee}/${options.state}/${options.area_of_employment}/${options.gender}/${options.type_of_Institution_where_you_are_currently_working}`
  ...
  useEffect(() => {
    setLoading(true)
    Promise.all([
      axios.get('https://dheeratest8.azurewebsites.net/api/v1/dheeraObgynPre/questions'),
    ]).then((responses) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get('https://dheeratest8.azurewebsites.net/api/v1/dheeraObgynPost/feedback'),
    ]).then((responses) => {
      return Promise.all(responses.map(response => {
        return response.data
      }))
  ...
  return (
    ...
    <Container>
       ...
       <PageTopSection 
      pageTitle="OBGYN Training Analytics"
      options={
        [
          {
            value : "obgyn_registration_pre", 
            name: "OBGYN Registration Pre"
          },
          {
            value : "dheera_obgyn_post_test_feedback", 
            name: "OBGYN Test Post Test Feedback"
          }
        ]
      }
      dateFilterUrl="https://dheeratest8.azurewebsites.net/api/v1/dheeraObgynPre/dateFilter"
      uploadUrl="https://dheeratest8.azurewebsites.net/api/v1/dheeraObgynPre/"
      setShow={setShow}
    ...
    { 
      ...
      <OBGYNFilter  handleFilter={handleFilter}/> 
      ...
      {
      csvfilename === "obgyn_registration_pre" && loading ? <div className='loader-container'>
          <Spinner animation="border" role="status">
            <span className="visually-hidden">Loading...</span>
          </Spinner> 
      </div> : !loading && message && csvfilename === "obgyn_registration_pre"?  
        <div className='loader-container'>
          <h3 className='error'>{message}</h3>
        </div> 
          : csvfilename === "obgyn_registration_pre" ? <DisplayChart 
    ...
    {
      csvfilename === "dheera_obgyn_post_test_feedback" && feedbackLoading ? 
      <div className='loader-container'>
          <Spinner animation="border" role="status">
            <span className="visually-hidden">Loading...</span>
          </Spinner> 
      </div>  :  
      !feedbackLoading && feedbackMSG  && csvfilename === "dheera_obgyn_post_test_feedback"? 
      <div className='loader-container'>
        <h3 className='error'>{feedbackMSG}</h3>
      </div> : csvfilename === "dheera_obgyn_post_test_feedback" ? 
      <DisplayChart 
...
success ? <Alert variant='success'>{success}</Alert> : null
      ...
      <Modal.Footer>
      {
          uploadERR || success ? null : <Button variant="primary" onClick={() => {
            ...
            let url = "https://dheeratest8.azurewebsites.net/api/v1/dheeraObgynPre"
            if(csvfilename === "dheera_obgyn_post_test_feedback"){
              url = "https://dheeratest8.azurewebsites.net/api/v1/dheeraObgynPost"
            }
            axios.post(url, data, {
