...
import {StudentFilter} from "../../utils/FilterCard";
...
const DheeraSchoolStudent = () => {
  
  const [show, setShow] = useState(false)
  const [csvfilename, setCSVfileName] = useState("dheera_school_student_registration_pre");
  ...
  function handleFilter(options){
    ...
    let url = `https://dheeratest8.azurewebsites.net/api/v1/dheeraSchoolStudentPre/filter/${options.age}/${options.state}/${options.select}/${options.school_type}/${options.class}/`
  ...
  // axios.get('https://dheeratest8.azurewebsites.net/api/v1/dheeraSchoolStudentPre/questions'),
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get('https://dheeratest8.azurewebsites.net/api/v1/dheeraSchoolStudentPost/feedback'),
    ]).then((responses) => {
      return Promise.all(responses.map(response => {
        return response.data
      }))
  ...
  return (
    ...
    <Container>
      ...
      <PageTopSection 
      pageTitle="Dheera School Student Training Analytics"
      options={
        [
          {
            value : "dheera_school_student_registration_pre", 
            name: "Dheera School Student Registration Pre"
          },
          {
            value : "dheera_school_student_post_test_feedback", 
            name: "Dheera School Strudent Test Post Test Feedback"
          }
        ]
      }
      dateFilterUrl="https://dheeratest8.azurewebsites.net/api/v1/dheeraSchoolStudentPre/dateFilter"
      uploadUrl="https://dheeratest8.azurewebsites.net/api/v1/dheeraSchoolStudentPre/"
      setShow={setShow}
    ...
    { 
      ...
      <StudentFilter  handleFilter={handleFilter}/> 
    ...
    {
      csvfilename === "dheera_school_student_registration_pre" && loading ? <div className='loader-container'>
          <Spinner animation="border" role="status">
            <span className="visually-hidden">Loading...</span>
          </Spinner> 
      </div> : !loading && message && csvfilename === "dheera_school_student_registration_pre"?  
        <div className='loader-container'>
          <h3 className='error'>{message}</h3>
        </div> 
          : csvfilename === "dheera_school_student_registration_pre" ? <DisplayChart 
    ...
    {
      csvfilename === "dheera_school_student_post_test_feedback" && feedbackLoading ? 
      <div className='loader-container'>
          <Spinner animation="border" role="status">
            <span className="visually-hidden">Loading...</span>
          </Spinner> 
      </div>  :  
      !feedbackLoading && feedbackMSG  && csvfilename === "dheera_school_student_post_test_feedback"? 
      <div className='loader-container'>
        <h3 className='error'>{feedbackMSG}</h3>
      </div> : csvfilename === "dheera_school_student_post_test_feedback" ? 
      <DisplayChart 
    ...
    <Modal.Footer>
        {
          uploadERR || success ? null : <Button variant="primary" onClick={() => {
            ...
            let url = "https://dheeratest8.azurewebsites.net/api/v1/dheeraSchoolStudentPre"
            if(csvfilename === "dheera_school_student_post_test_feedback"){
              url = "https://dheeratest8.azurewebsites.net/api/v1/dheeraSchoolStudentPost"
            }
            axios.post(url, data, {
...
export default DheeraSchoolStudent
