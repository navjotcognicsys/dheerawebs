var Dheera_school_student_pre_Schema = require('../models/dheera_school_student_pre.js')
var Dheera_school_student_post_feedback_Schema = require('../models/dheera_school_student_post_feedback.js')
...
function matchQuestions(items, postItems) {
    ...
    console.log(items?.length, postItems?.length)
...
router.get('/questions', (req, res) => {
    Dheera_school_student_post_feedback_Schema.find({}, (err, items) => {
        if(err){
            console.log(err)
        }else {
            let postFeedbackAll = items

            Dheera_school_student_pre_Schema.find({}, (err, items) => {
                if (err) {
                    console.log(err);
                }
                else {
...
router.get('/dateFilter/:startDate/:endDate', (req, res) => {
    ...
    Dheera_school_student_pre_Schema.find({"registration_time":{"$gte": (req.params.startDate), "$lt": (req.params.endDate)}}, (err, items) => {
        if (err) {
            console.log(err);
        }
        else {
...
async function findUsersInPostFeedback(preItems) {
    ...
    for (const item of preItems) {
        ...
        try {
            let found = await Dheera_school_student_post_feedback_Schema.findOne({"Username" : item.email })
            if(found){
...
router.post('/', upload.single('dheera_school_student_registration_pre'), (req, res, next) => {
    ...
    .then((jsonObj)=>{
        if(compareKeys(jsonObj)){
            ...
            for(var i = 0;i<jsonObj.length;i++){
                ...
                Dheera_school_student_pre_Schema.insertMany(army).then(function(){
                    console.log(army)
                    res.status(200).send({
...
router.get('/get/count', async (req, res) =>{
    const userCount = await Dheera_school_student_pre_Schema.estimatedDocumentCount()
